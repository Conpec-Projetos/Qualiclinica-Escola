...
export default function CourseModel({
    ...
    const hasInterestLink = Boolean(interestLink?.trim());

    const handleGoToInterest = () => {
        const link = interestLink?.trim();
        if (!link) {
    ...
    return (
        <div className="flex flex-col lg:flex-row bg-rosa-claro rounded-[5px] p-4 md:p-[1.375rem] gap-4 w-full min-h-[200px] lg:h-[300px]">
                ...
                <div className="flex flex-col h-full items-start justify-between gap-4">
                        ...
                        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
                            ...
                            <CollapsibleContent>
                                <div className="w-full max-h-48 overflow-y-auto hover:bg-white mt-2 p-2">
                                    <p className="text-text text-sm font-medium whitespace-pre-wrap">{description}</p>
                                </div>
                            </CollapsibleContent>
                        </Collapsible>
                    </div>
                    ...
                    <div className="flex flex-row gap-3 flex-wrap">
                        <ButtonQuali
                            ...
                            fontSize="large"
                            onClick={hasInterestLink ? handleGoToInterest : () => router.push("/contate-nos")}
                        />
                        ...
                        {hasInterestLink && (
                            <ButtonQuali
                                text="TENHO DÚVIDAS"
                                fontSize="large"
                                onClick={() => router.push("/contate-nos")}
                            />
                        )}
                    </div>
                </div>
