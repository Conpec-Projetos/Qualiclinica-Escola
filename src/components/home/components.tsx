...
export function ButtonHomeModel({
  ...
  if (text == "Conheça-nos") {
    ...
    return (
      <button
        onClick={() => router.push(path)}
        className="h-[3.125rem] w-[15rem] sm:w-fit flex gap-[4px] items-center px-[1rem] bg-magenta rounded-[5px] hover:cursor-pointer"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          height="30px"
        ...
        <p className="text-white w-full text-center text-[20px] sm:text-[15px] uppercase">{text}</p>
      </button>
    );
  }
}
